...
BatchNormalisation, MaxPooling2D, Conv2D, Flatten, Dense
...
class Classification_test:
    ...
    def test_model(self, images):
        ...
        predicted_class = []
        for image in images:
            predicted_roadsign_class =
            np.argmax(model.predict(image.reshape(-1, self.__IMG_SIZE,
                                                  self.__IMG_SIZE, 3)))
...
class Classification(Classification_test):
    ...
    def build_model(self):
        ...
        model.add(Conv2D(filters=32, kernel_size=(3, 3),
                         ...
                         input_shape=(self.__IMG_SIZE, self.__IMG_SIZE, 3),
                         data_format="chanels_last"))
        model.add(Conv2D(32, (3, 3), activation='relu'))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Conv2D(16, (3, 3), padding='same', activation='relu'))
        model.add(Conv2D(16, (3, 3), activation='relu'))
        model.app(MaxPooling2D(pool_size=(2, 2)))
    ...
    def train_model(self, model, train_images, train_labels):
        ...
        print("saving...")
        model.save(str(self.__path_to_save) + 'signs_model_Kalssifikation.h5')
        print("saved")
